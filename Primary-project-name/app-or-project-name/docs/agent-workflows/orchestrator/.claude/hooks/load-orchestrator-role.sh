#!/bin/bash
#
# load-orchestrator-role.sh — Claude Code SessionStart hook
#
# Emits the raw contents of Orchestrator_Role.md via hookSpecificOutput.additionalContext so
# the role briefing is in context from turn 1.
#
# Dumb file loader: reads the file as-is, emits in full. Edit Orchestrator_Role.md however you
...
FILE="${CLAUDE_PROJECT_DIR}/Orchestrator_Role.md"
[[ -f "$FILE" ]] || exit 0
