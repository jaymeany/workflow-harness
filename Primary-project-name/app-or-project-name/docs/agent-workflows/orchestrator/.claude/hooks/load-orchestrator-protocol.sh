#!/bin/bash
#
# load-orchestrator-protocol.sh — Claude Code SessionStart hook
#
# Emits the raw contents of protocol/Orchestrator_Protocol.md via hookSpecificOutput.additionalContext so
# the planning methodology is in context from turn 1.
#
# Dumb file loader: reads the file as-is, emits in full. Edit protocol/Orchestrator_Protocol.md however you
...
FILE="${CLAUDE_PROJECT_DIR}/protocol/Orchestrator_Protocol.md"
[[ -f "$FILE" ]] || exit 0
