#!/bin/bash
#
# load-research-trello-catchup.sh — Claude Code SessionStart hook
#
...
# own first poll, by the word "research" in the list name, never an exact
# label — same rule as is_research_column in lib.sh.
#
...
# tell the session not to arm, silently leaving this project with no watcher.
#
...
WATCH_TAG="trello-list-watch-research-{{PROJECT_SLUG}}"
...
Trello column for this role: the board's research column, whichever list
name contains \"research\". Do not treat any exact label as canonical; the
gates match on the word, not the full string.
...
don't land (verify with 'pgrep -f ${WATCH_TAG}'). Never run
two: duplicates double-fire every arrival."
  else
...
arrivals in Research column on the {{PROJECT_NAME}} board\", and this command:

$(board_watch_command "$WATCH_TAG" "$TRELLO_BOARD_ID" "$(board_column_regex research)")
