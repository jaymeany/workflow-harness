#!/bin/bash
#
# load-designer-trello-catchup.sh — Claude Code SessionStart hook
#
...
# own first poll, by the WORD "design" in the list name, never an exact label.
...
WATCH_TAG="trello-list-watch-designer-{{PROJECT_SLUG}}"
...
Trello column for this role: the board's design column, whichever list
name contains the word \"design\". Do not treat any exact label as canonical;
...
arrivals in {{PROJECT_NAME}} Design column\", and this command:

$(board_watch_command "$WATCH_TAG" "$TRELLO_BOARD_ID" "$(board_column_regex design)")
