#!/bin/bash
#
# load-qa-workflow.sh — Claude Code SessionStart hook
#
# Emits the raw contents of protocol/QA_Workflow.md via
# hookSpecificOutput.additionalContext so the workflow methodology
# (unit testing, iteration, hotfix, team composition, E2E, what NOT
# to flag, documentation outputs) is in context from turn 1.
#
# Dumb file loader: reads the file as-is, emits in full. Does not
# parse the doc or know its structure. Edit protocol/QA_Workflow.md
...
FILE="${CLAUDE_PROJECT_DIR}/protocol/QA_Workflow.md"
