#!/bin/bash
#
# load-qa-trello-catchup.sh — Claude Code SessionStart hook
#
...
# own first poll, by the word "qa" in the list name, never an exact label.
#
# The watcher's curl is a read-only list poll inside the Monitor shell —
# sanctioned infra read. Card OPERATIONS still go
# through mcp__trello__* only; deny-direct-trello-api.sh gates Bash.
#
...
Trello column for this role: the board's qa column, whichever list
name contains \"qa\". Do not treat any exact label as canonical; the gates
...
- The curl is a read-only list poll inside the Monitor shell — sanctioned
  infra read. Card OPERATIONS still go through
  mcp__trello__* ONLY; the Bash gate on api.trello.com stands.
